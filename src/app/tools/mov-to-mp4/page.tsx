...
export default function MovToMp4Page() {
    ...
    const [outputFilename, setOutputFilename] = useState("converted.mp4");
    const [ffmpegLoaded, setFfmpegLoaded] = useState(false);
    const ffmpegRef = useRef<FFmpeg | null>(null);
    const inputRef = useRef<HTMLInputElement>(null);

    const relatedTools = tools
        .filter((t) => t.category === "video" && t.id !== "mov-to-mp4")
        .slice(0, 4);
    ...
    const handleFiles = useCallback((fileList: FileList | null) => {
        ...
        if (ext !== 'mov') {
            setErrorMsg("Select an Apple MOV file (.mov).");
            return;
        }
    ...
    const convert = async () => {
        ...
        try {
            ...
            // Standard MOV to MP4 conversion often works via stream copy
            // if the original is H.264/AAC.
            ...
            setOutputFilename(file.name.replace(/\.mov$/i, "") + ".mp4");
            setDownloadUrl(url);
            setProgress(100);
            setConvState("done");
        ...
        <div className="min-h-screen pt-24 pb-24 px-4 overflow-x-hidden">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-12 text-center sm:text-left">
                    <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-sky-500 to-blue-700 flex items-center justify-center shadow-2xl flex-shrink-0 animate-in fade-in zoom-in">
                        <PlayIcon className="w-10 h-10 text-white fill-white" />
                    </div>
                    <div className="flex-1">
                        <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tight">MOV to MP4</h1>
                        <p className="text-[#9090b0] mt-3 text-lg leading-relaxed max-w-2xl text-balance">
                            Instantly convert Apple <strong className="text-white">MOV</strong> videos to the universal <strong className="text-white">MP4</strong> format.
                        </p>
                    </div>
                </div>
                    ...
                    {(convState === "idle" || convState === "error") && (
                        <div className="animate-in fade-in duration-500">
                            <div
                                ...
                                className={`relative flex flex-col items-center justify-center gap-6 p-12 sm:p-20 rounded-[1.5rem] cursor-pointer transition-all duration-500 border-2 border-dashed group/dropzone ${dragging ? "border-sky-500 bg-sky-600/10" : "border-white/10 bg-white/[0.02] hover:border-sky-500/40"}`}
                            >
                                <input ref={inputRef} type="file" accept="video/quicktime" className="hidden" onChange={(e) => handleFiles(e.target.files)} />
                                {file ? (
                                    ...
                                    <div className="flex flex-col items-center gap-4 text-center">
                                        <div className="w-16 h-16 rounded-2xl bg-sky-500/10 flex items-center justify-center transition-transform group-hover/dropzone:scale-110">
                                            <Upload className="w-8 h-8 text-sky-400" />
                                        </div>
                                        <p className="text-white font-black text-2xl tracking-tight">Drag & drop MOV</p>
                            ...
                            <button onClick={convert} disabled={!file || !ffmpegLoaded} className={`mt-8 w-full py-5 rounded-xl font-black text-white text-lg transition-all ${file && ffmpegLoaded ? "bg-gradient-to-r from-sky-600 to-blue-700 hover:from-sky-500 shadow-xl shadow-sky-900/40" : "bg-white/5 text-[#404060] cursor-not-allowed"}`}>
                                {file ? "Convert to MP4" : "Select Video File"}
                            </button>
                        </div>
                    )}
                    ...
                    {convState === "processing" && (
                        <div className="py-12 flex flex-col items-center gap-8 animate-in fade-in duration-500">
                            <div className="relative w-32 h-32 flex items-center justify-center">
                                <Loader2 className="w-10 h-10 text-sky-500 animate-spin absolute z-10" />
                                <div className="text-2xl font-black text-white z-20">{progress}%</div>
                            </div>
                            <h3 className="text-3xl font-black text-white tracking-tight">Processing QuickTime Stream...</h3>
                            <div className="w-full max-w-sm bg-white/5 rounded-full h-3 p-1">
                                <div className="h-full rounded-full bg-gradient-to-r from-sky-500 to-blue-700 shadow-[0_0_10px_rgba(14,165,233,0.5)] transition-all duration-300" style={{ width: `${progress}%` }} />
                            </div>
                        </div>
                    )}
                    ...
                    {convState === "done" && downloadUrl && (
                        <div className="py-10 flex flex-col items-center gap-8 animate-in zoom-in fade-in">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-[#9090b0] text-lg mt-4 font-medium max-w-sm mx-auto">Your MP4 file is ready for download.</p>
                            </div>
                            <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md mt-4">
                                <button onClick={triggerDownload} className="flex-1 flex items-center justify-center gap-3 py-5 bg-gradient-to-r from-sky-600 to-blue-600 hover:scale-[1.02] text-white font-black text-lg rounded-2xl transition-all shadow-xl shadow-sky-900/30">
...
// Removing redundant lucide-react manual imports that were accidentally added
