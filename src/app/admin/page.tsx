...
export default function AdminDashboard() {
  ...
  const filteredConversions = data?.recentConversions.filter(conv =>
          ...
          <div className="glass p-10 rounded-[3rem] border border-white/10">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-[#606080] text-[10px] font-black uppercase tracking-widest mt-1">Top tools by total conversions</p>
              </div>
              <div className="flex items-center gap-4">
                <Link
                  href="/admin/tools"
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-violet-400 text-xs font-black uppercase tracking-widest hover:bg-white/10 transition-all"
                >
                  See All
                ...
                <div key={tool.name} className="flex items-center justify-between p-5 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[#606080] text-[10px] font-black uppercase tracking-widest">
                        {tool.share}% of all conversions
                      </p>
                    </div>
                  </div>
          ...
          <div className="p-10 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <p className="text-[#606080] text-[10px] font-black uppercase tracking-widest">Platform-wide audit log</p>
                  <span className="w-1 h-1 rounded-full bg-[#404060]" />
                  <p className="text-violet-400 text-[10px] font-black uppercase tracking-widest">
                    {filteredConversions.length} {filteredConversions.length === 1 ? 'Record' : 'Records'} Found
                  </p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="relative group">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#606080] group-focus-within:text-violet-400 transition-colors" />
                <input
                  type="text"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  placeholder="Search by user or file..."
                  className="bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-2 text-xs text-white placeholder-[#404060] focus:outline-none focus:ring-1 focus:ring-violet-500/50 transition-all w-64"
                />
              </div>
